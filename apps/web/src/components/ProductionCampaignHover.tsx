...
function validDecision(
	...
	const deadline = Math.min(
		Date.parse(decision.authorizationExpiresAt),
		Date.parse(decision.endsAt),
		Date.parse(decision.serverTime) + MAX_LEASE_MS,
	);
	if (
		!decision.activityId ||
...
export function ProductionCampaignHover({
	...
	useEffect(() => {
		...
		const current = (requestGeneration: number) =>
			!cancelled && requestGeneration === requestGenerationRef.current;
		const decide = async () => {
			...
			try {
				...
				const [entryLoaded, layerLoaded] = await Promise.all([
					loadProductionTouchpointDecision(
						...
						locale,
						controller.signal,
						mounted?.entry.decision.touchpointDecisionId,
					),
					loadProductionTouchpointDecision(
						LAYER_PLACEMENT,
						locale,
						controller.signal,
						mounted?.layer.decision.touchpointDecisionId,
					),
				]);
				if (!current(requestGeneration)) return;
				const matchesMounted = (
				...
				if (
					matchesMounted(entryLoaded, mounted?.entry.decision) ||
					matchesMounted(layerLoaded, mounted?.layer.decision)
				) {
					clear();
					return;
				}
				...
				if (
					entryLoaded.kind === "no-decision" ||
					layerLoaded.kind === "no-decision"
				) {
					if (!mounted) clear();
					return;
				}
				...
				if (
					!entry ||
					!layer ||
					entry.decision.activityId !== layer.decision.activityId ||
					entry.decision.deploymentId !== layer.decision.deploymentId ||
					readCampaignHostLocale() !== locale
				) {
					clear();
					return;
				}
				...
				// Renewed credentials replace action authority without replacing immutable content.
				...
				timer = setTimeout(
					() => {
						if (expiryRef.current === expiresAt) clear();
					},
	...
	}, [authenticated, clear, locale, sessionSubject, testRuntime]);
	...
	if (authenticated && testRuntime && testEntry && testLayer) {
		return (
			<HoverTouchpointOverlay
				...
				mountIdentity={`test:${testRuntime.deployment.id}`}
			/>
		);
	}
	return authenticated && active?.sessionSubject === sessionSubject ? (
		<HoverTouchpointOverlay
			...
			mountIdentity={`production:${active.entry.decision.activityId}:${active.entry.decision.deploymentId}:${active.layer.decision.deploymentId}`}
		/>
	) : null;
}
