...
// OPEND-2745: a completed memory extraction must no longer append a ChatPanel notification.
...
describe('ProjectView OPEND-2745 stops creating memory notification messages', () => {
  beforeEach(() => {
  ...
  it('does not append or persist a host memory card after the active conversation has loaded', async () => {
    ...
    memory.batch = { key: 'completed-extraction-2745', count: 1,
    ...
    expect(view.container.textContent).not.toContain('OPEND2745_STORED_RULE');
    expect(view.container.textContent).not.toContain('<od-card');
    expect(vi.mocked(saveMessage).mock.calls.slice(persistedBefore)).toEqual([]);
