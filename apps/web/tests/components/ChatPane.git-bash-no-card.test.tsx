...
describe('Git Bash dependency failure keeps its run diagnosis without a recovery card', () => {
  ...
  it('keeps the ordinary missing-CLI card and its working retry action', () => {
    ...
    expect(onRetry).toHaveBeenCalledWith(expect.objectContaining({ id: message.id }), 'manual_retry');
    expect(message).toEqual(original);
  });
});
